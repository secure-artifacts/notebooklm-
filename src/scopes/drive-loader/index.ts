import { notebookOrigins } from "@/const";
  ...
  async function downloadPublicDriveMedia(sharedUrl, requestId, parentOrigin) {
    ...
    const blobParts = chunks as BlobPart[];
    const preliminaryBlob = new Blob(blobParts, { type: contentType || "application/octet-stream" });
    ...
    return new File(blobParts, fileName, { type: detected.mimeType, lastModified: Date.now() });
  }
