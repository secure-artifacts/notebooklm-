...
> 当前版本：`v0.13.2` · 发布后下载并解压 Release 中的 ZIP，在浏览器扩展管理页使用「加载已解压的扩展程序」导入。

## 最新更新

### v0.13.2

- 修复 AI 返回相同译文时被当作旧回复过滤、导致一直显示翻译中的问题；改为按唯一请求标记关联本次对话。
- 保存翻译请求与来源行的关联，恢复时优先读取已有回答；无法确认归属时暂停并保留来源。
- 区分等待、生成和校验阶段，排除思考区与引用标记，结合文本稳定性、生成状态和回答操作按钮判断完成。
- 对结束后的无效 JSON 明确报错，有限兼容字段名中的 Markdown 转义，并拒绝歧义的重复来源项。
- 更新后重新加载扩展并刷新 NotebookLM；旧版本卡住的行需要勾选后重试一次，以建立新的请求关联。
