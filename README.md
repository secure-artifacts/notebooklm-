...
> 当前版本：`v0.13.3` · 发布后下载并解压 Release 中的 ZIP，在浏览器扩展管理页使用「加载已解压的扩展程序」导入。

## 最新更新

### v0.13.3

- 修复来源名称按列表顺序错配的问题，提取和翻译选择改为按 `sourceId` 对应；翻译前校正可确认的旧缓存名称。
- 重名来源分开翻译，保存每次请求的行、来源和名称关联，拒绝歧义回复。
- 加强暂停与异步回写保护，避免旧任务覆盖当前任务；无法确认上传结果时暂停，避免盲目重复导入。
- 修复首次进入笔记本的记录归属校验，以及清除部分未开始行后空白行阻塞导入的问题。
- 登记成功必须有对应行的有效回执，缺失或冲突时不自动删除来源。
- 更新后重新加载扩展并刷新 NotebookLM。已完成自动化测试与 Edge 只读身份核验；大批量连续运行、重名翻译和暂停恢复仍需实际场景验证，建议先关闭自动移除来源做小批量测试。
